...
Container::Container() : gSize(0)
{
    ...
    std::string name; std::vector<std::string> players;
    while (file >> name)
...
const Player& Container::getPlayer(const std::string &name) const noexcept
{
    for (auto iter = gFirst->pTarget; iter != gFirst.get(); iter = iter->pTarget)
        if (iter->pName == name) return *iter;
...
{
    return const_cast<Player&>(const_cast<const Container*>(this)->getPlayer(name));
}
...
void Container::kill(const std::string &name)
{
    ...
    while (currNode->pName != name) currNode = currNode->pTarget;
...
void Container::assignTargets(std::vector<std::string> &players)
{
    ...
    auto forward = firstNode->pTarget; uint32_t fCount = 0;
    for (; forward != firstNode; forward = forward->pTarget)
        ++fCount;
    auto backward = firstNode->pHunter; uint32_t bCount = 0;
